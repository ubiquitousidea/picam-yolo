...
# WEIGHTS drives both the export and the directory it lands in, because
# export_model.py names the output after the checkpoint stem. Deriving MODEL_DIR
# from the same variable keeps the two in step; hardcoding the directory while
# passing only --imgsz to the export made them agree by coincidence alone.
WEIGHTS="${WEIGHTS:-yolo11n.pt}"
MODEL_NAME="${MODEL_NAME:-$(basename "$WEIGHTS" .pt)_ncnn_model}"
...
if [[ "${SKIP_MODEL:-0}" == "1" ]]; then
  ...
  echo "==> exporting $WEIGHTS at imgsz=$IMGSZ (single core; takes a few minutes)"
  ( cd "$REPO_DIR" && OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
      taskset -c 0 nice -n 10 "$VENV/bin/python" scripts/export_model.py \
        --weights "$WEIGHTS" --imgsz "$IMGSZ" --outname "$MODEL_NAME" )
...
echo
echo "Run the server:"
echo "  cd $REPO_DIR && .venv/bin/python -m picam_yolo.server --model models/$MODEL_NAME --imgsz $IMGSZ"
